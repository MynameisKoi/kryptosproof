...
async def probe_target_endpoints(ctx: RunContext[AttackScriptDeps]) -> dict:
    ...
    data = await probe_endpoints(ctx.deps.target_url)
    return tool_return("probe_target_endpoints", data, detail=ctx.deps.target_url)
...
async def check_security_headers(ctx: RunContext[AttackScriptDeps]) -> dict:
    ...
    data = await get_security_headers(ctx.deps.target_url)
    return tool_return("check_security_headers", data, detail=ctx.deps.target_url)
...
async def detect_target_technologies(ctx: RunContext[AttackScriptDeps]) -> dict:
    ...
    data = await detect_technologies(ctx.deps.target_url)
    return tool_return("detect_target_technologies", data, detail=ctx.deps.target_url)
...
async def extract_forms(ctx: RunContext[AttackScriptDeps], path: str = "/") -> dict:
    ...
    data = await get_forms(url)
    return tool_return("extract_forms", data, detail=url)
...
async def nuclei_scan(
    ...
    """
    data = await run_nuclei_scan(ctx.deps.target_url, tags=tags, severity=severity)
    return tool_return("nuclei_scan", data, detail=ctx.deps.target_url)
...
async def ffuf_fuzz_directories(
    ...
    """
    data = await run_ffuf_directory_fuzz(ctx.deps.target_url, wordlist_path=wordlist_path)
    return tool_return("ffuf_fuzz_directories", data, detail=ctx.deps.target_url)
...
async def sqlmap_scan(ctx: RunContext[AttackScriptDeps], url_with_parameter: str) -> dict:
    ...
    """
    data = await run_sqlmap_probe(url_with_parameter)
    return tool_return("sqlmap_scan", data, detail=url_with_parameter)
...
async def pat_search_payload_files(ctx: RunContext[AttackScriptDeps], keyword: str) -> dict:
    ...
    """
    data = search_payload_files(keyword)
    return tool_return("pat_search_payload_files", data, detail=keyword)
...
async def pat_read_payload_lines(
    ...
    data = read_payload_lines(relative_path, max_lines=max_lines)
    return tool_return("pat_read_payload_lines", data, detail=relative_path)
...
async def zap_status(ctx: RunContext[AttackScriptDeps]) -> dict:
    ...
    data = await zap_ping()
    return tool_return("zap_status", data, detail=ctx.deps.target_url)
...
async def zap_spider_scan(ctx: RunContext[AttackScriptDeps]) -> dict:
    ...
    """
    data = await zap_spider_and_alerts(ctx.deps.target_url)
    return tool_return("zap_spider_scan", data, detail=ctx.deps.target_url)
...
async def zap_aggressive_active_scan(ctx: RunContext[AttackScriptDeps]) -> dict:
    ...
    """
    data = await zap_active_scan(ctx.deps.target_url)
    return tool_return("zap_aggressive_active_scan", data, detail=ctx.deps.target_url)
